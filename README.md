...
实现 `backend/news.py`：在仿真开始时从 BBC/NYT RSS 抓取今日新闻标题，用 Claude 创意映射成公寓内能发生的具体事件（断电、手机弹亏损提醒、酒瓶叮当响…），注入 `pending_events` 队列，下一 tick 自动触发。

映射原则：
- 事件必须在公寓内**物理可发生**（不能凭空出现剧情外元素）
- 大事件（importance ≥ 8）每晚最多 1 个，防止剧情失控
- 每次映射返回 1-3 个事件，由 LLM 自主选取最有戏剧张力的标题

这一步让每次运行的除夕夜都不一样，剧情由真实世界当天的新闻随机染色。
